...
class AccountVatPeriodEndStatement(models.Model):
    ...
    def _clean_group_total(self):
        ...
        for model in (self.env['statement.debit.group.line'],
    ...
    def evaluate_tax_values(self, tax, statement):
        ...
        for period in statement.date_range_ids:
            total_base, total_vat, total = map(sum,
    ...
    def _set_group_lines(self, type):
        ...
        if type and type in ('credit', 'debit'):
            lines = self.debit_vat_account_line_ids if type == 'debit' \
            ...
            for line in lines:
                if line.account_id:
                    ...
                    if account in accounts:
                        ...
                        accounts[account]['undeductible_amount'] += \
                            line.undeductible_amount
                        accounts[account]['deductible_amount'] += \
                    ...
                    if kind_id in kinds:
                        ...
                        kinds[kind_id]['undeductible_amount'] += \
                            line.undeductible_amount
                        kinds[kind_id]['deductible_amount'] += \
                            line.deductible_amount
                    else:
                        kinds[kind_id] = {
                            'amount': line.amount,
                    ...
                    if 'unique' in others:
                        ...
                        others['unique']['undeductible_amount'] += \
                            line.undeductible_amount
                        others['unique']['deductible_amount'] += \
                            line.deductible_amount
                    else:
                        others.update({'unique': {
                            'amount': line.amount,
                            'type': 'other',
                            'base_amount': line.base_amount,
                            'vat_amount': line.vat_amount,
    ...
    def _set_group_totals(self, totals, type):
        model = self.env['statement.debit.group.line'] if type == 'debit' \
        ...
        for total, value in totals.items():
            ...
            if value['type'] and value['type'] == 'account':
                vals.update({'account_id': total.id,
                             'name': total.display_name})
            # end if

            if value['type'] and value['type'] == 'nature':
                vals.update({'kind_id': total.id,
                             'name': total.display_name})
            # end if
...
class StatementDebitAccountLine(models.Model):
    ...
    def _compute_deductible_amount(self):
        for line in self:
            if line.tax_id.eu_vat:
                ...
                _logger.info('is_split_payment {sp}'.format(sp=is_split_payment))
                if not line.tax_id.account_id or is_split_payment or (
    ...
    deductible_amount = fields.Float(
        'Amount', compute='_compute_deductible_amount', store=True,
    )

    undeductible_amount = fields.Float(
        'Undeductible', compute='_compute_undeductible_amount', store=True,
    )
...
class StatementCreditAccountLine(models.Model):
    ...
    deductible_amount = fields.Float(
        'Iva ', compute='_compute_deductible_amount', store=True,
    )
